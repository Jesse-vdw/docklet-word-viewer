...
export class WordViewerBridge {
	...
	mount(): Promise<void> {
		...
		this.iframe.srcdoc = buildWordViewerHtml();
	...
	loadDocument(documentModel: WordDocumentModel, isDark: boolean, layout: WordLayoutMode): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'loadDocument', document: documentModel, isDark, layout });
	}

	setTheme(isDark: boolean): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'themeChanged', isDark });
	}

	setLayout(layout: WordLayoutMode): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'layoutChanged', layout });
	}

	setZoom(zoom: number): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'zoomChanged', zoom });
	}

	setSearch(query: string): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'searchChanged', query });
	}

	navigateSearch(direction: 'next' | 'previous'): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'searchNavigate', direction });
	}

	scrollToBlock(blockId: string): void {
		this.post({ channel: BRIDGE_CHANNEL, type: 'scrollToBlock', blockId });
	}
