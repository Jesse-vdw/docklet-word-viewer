...
export interface NumberingDefinitions {
	...
	abstractLevels: Map<string, Map<number, NumberingLevelDefinition>>;
}
...
export class DocxPartParser {
	...
	parseStyles(bytes: Uint8Array | undefined): Map<string, WordStyleDefinition> {
		...
		for (const style of children(parseXml(decodeUtf8(bytes), STYLES_XML_PATH).documentElement, 'style')) {
			...
			styles.set(styleId, {
				...
				headingLevel:
					parseOutlineLevel(attr(first(style, 'outlineLvl'), 'val')) ?? headingLevelFromName(name ?? styleId),
			});
		}
		return styles;
	}
	...
	parseNumbering(bytes: Uint8Array | undefined): NumberingDefinitions {
		...
		for (const num of children(xml.documentElement, 'num')) {
			const numId = attr(num, 'numId'),
				abstractId = attr(child(num, 'abstractNumId'), 'val');
			if (numId && abstractId) {
				definitions.numToAbstract.set(numId, abstractId);
			}
		}
		return definitions;
	}
	...
	parseMetadata(bytes: Uint8Array | undefined): WordDocumentMetadata {
		...
		return Object.fromEntries(
			['title', 'subject', 'creator', 'description', 'created', 'modified'].map((key) => [key, textOfFirst(root, key)]),
		) as unknown as WordDocumentMetadata;
	}
}
...
export function toListFormat(value: string | null): WordListFormat {
	return (['bullet', 'lowerLetter', 'upperLetter', 'lowerRoman', 'upperRoman', 'none'] as WordListFormat[]).includes(
		value as WordListFormat,
	)
		? (value as WordListFormat)
		: 'decimal';
}
