...
const BLOCKED_PUBLIC_HOSTS = new Set([
	'services.syncfusion.com',
	'ej2.syncfusion.com',
	'document.syncfusion.com',
]);
...
export class SyncfusionConversionClient {
	async convertToSfdt(read: WordReadResult, settings: WordViewerSettings): Promise<string> {
		...
		formData.append('files', new Blob([read.data], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' }), read.file.name);
		...
		try {
			...
			if (!response.ok) {
				throw new WordViewerDomainError('REMOTE_CONVERSION_FAILED', `Remote conversion failed with HTTP ${response.status}.`);
			}
			const sfdt = await response.text();
			if (!sfdt.trim()) {
				throw new WordViewerDomainError('REMOTE_CONVERSION_FAILED', 'Remote conversion returned an empty SFDT document.');
			}
			return sfdt;
		} catch (error) {
