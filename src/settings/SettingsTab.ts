...
export class DockletWordViewerSettingTab extends PluginSettingTab {
	constructor(
		app: App,
		private readonly host: WordViewerSettingsHost,
	) {
...
export function renderSettingsSection(
	...
	new Setting(containerEl)
		...
		.addDropdown((dropdown) =>
			dropdown
				...
				.onChange((value) =>
					patch(host, { defaultLayout: value === 'continuous' ? 'continuous' : 'paginated' }, refresh),
				),
		);
	numberSetting(
		containerEl,
		'Maximum local file size',
		'Block very large documents before parsing them in the Obsidian renderer process.',
		String(MIN_LOCAL_FILE_SIZE_MB),
		String(settings.maxFileSizeMb),
		MIN_LOCAL_FILE_SIZE_MB,
		MAX_LOCAL_FILE_SIZE_MB,
		(value) => patchNumber(host, 'maxFileSizeMb', value),
	);
	toggleSetting(
		containerEl,
		'Allow remote conversion',
		'Enable posting documents to a configured self-hosted Syncfusion-compatible conversion service. Leave off to keep documents local.',
		settings.allowRemoteConversion,
		(value) => {
			...
			if (patchValue.syncfusionServiceUrl) {
				serviceUrlInput?.setValue(patchValue.syncfusionServiceUrl);
			}
	...
	preferRemoteToggle = toggleSetting(
		containerEl,
		'Prefer remote conversion',
		'Use the configured self-hosted converter before the local parser. Otherwise it is only used as a fallback after local parsing fails.',
		settings.preferRemoteConversion,
		(value) => patch(host, { preferRemoteConversion: value }, refresh),
	);
	timeoutInput = numberSetting(
		containerEl,
		'Remote conversion timeout',
		'Abort self-hosted conversion requests after this many milliseconds.',
		'15000',
		String(settings.remoteConversionTimeoutMs),
		MIN_REMOTE_CONVERSION_TIMEOUT_MS,
		MAX_REMOTE_CONVERSION_TIMEOUT_MS,
		(value) => patchNumber(host, 'remoteConversionTimeoutMs', value),
	);
	updateRemoteDependentControls();
}
...
function patchNumber(
	host: WordViewerSettingsHost,
	key: 'maxFileSizeMb' | 'remoteConversionTimeoutMs',
	value: string,
): number | null {
	if (value.trim() === '') {
		return null;
