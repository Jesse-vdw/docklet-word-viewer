...
import { WordFileRepository } from '../services/WordRepositoryService.ts';
...
export class WordViewerView extends ItemView {
	...
	private buildToolbar(toolbar: HTMLElement): void {
		for (const action of this.toolbarActions()) {
			...
			if (action.label === this.layoutButtonLabel()) { this.layoutButtonEl = button; }
		}
		const search = toolbar.createDiv({ cls: C.CSS_SEARCH });
		this.searchInputEl = search.createEl('input', { cls: C.CSS_SEARCH_INPUT, attr: { type: 'search', placeholder: 'Search' } });
		this.searchInputEl.addEventListener('input', () => this.bridge.setSearch(this.searchInputEl?.value ?? ''));
		this.searchInputEl.addEventListener('keydown', (event) => {
			if (event.key === 'Enter') { this.bridge.navigateSearch(event.shiftKey ? 'previous' : 'next'); }
		});
		this.createToolbarButton(search, { icon: 'chevron-up', label: 'Previous search result', onClick: () => this.bridge.navigateSearch('previous') });
		this.createToolbarButton(search, { icon: 'chevron-down', label: 'Next search result', onClick: () => this.bridge.navigateSearch('next') });
		this.searchStatusEl = search.createDiv({ cls: C.CSS_SEARCH_STATUS, text: '0/0' });
	}

	private toolbarActions(): ToolbarAction[] {
		return [
			{ icon: 'refresh-cw', label: 'Reload document', onClick: () => { void this.reloadDocument(); } },
			...
			{ icon: 'copy', label: 'Copy document text', onClick: () => { void this.copyDocumentText(); } },
			{ icon: 'external-link', label: 'Open in default app', onClick: () => { void this.openExternally(); } },
		];
	}
	...
	private updateLayoutButton(): void {
		if (this.layoutButtonEl) { setToolbarButtonIcon(this.layoutButtonEl, this.layoutButtonIcon(), this.layoutButtonLabel()); }
	}

	private layoutButtonIcon(): string { return this.layout === 'paginated' ? 'rows-3' : 'files'; }
	...
	private async openExternally(): Promise<void> {
		...
		try {
			const fullPath = (this.app.vault.adapter as { getFullPath?: (path: string) => string }).getFullPath?.(this.currentFile.path);
			if (!fullPath) {
			...
			if (typeof result === 'string' && result.length > 0) { new Notice(`${C.PLUGIN_NAME}: ${result}`); }
		} catch (error) {
