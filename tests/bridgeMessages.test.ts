...
describe('isWordToHostMessage', () => {
	it('accepts ready and render error messages for the viewer channel', () => {
		...
		expect(isWordToHostMessage({ channel: BRIDGE_CHANNEL, bridgeId: 'bridge-1', type: 'renderError', message: 'Nope' })).toBe(true);
		expect(isWordToHostMessage({ channel: BRIDGE_CHANNEL, bridgeId: 'bridge-1', type: 'searchResult', total: 3, active: 1 })).toBe(true);
	});
	...
	it('rejects messages from other channels or malformed payloads', () => {
		...
		expect(isWordToHostMessage({ channel: BRIDGE_CHANNEL, bridgeId: 'bridge-1', type: 'searchResult', total: '3', active: 1 })).toBe(false);
