import { DocxParser } from '../../src/services/docx/DocxParser.ts';
import { isWordViewerDomainError } from '../../src/domain/WordViewerDomainError.ts';
...
describe('DocxParser', () => {
	it('parses paragraphs, formatting, and tables from a minimal docx package', () => {
		...
		expect(model.blocks[2]).toMatchObject({
			...
			rows: [
				{
					cells: [
						{ widthPt: 72, blocks: [{ type: 'paragraph' }] },
						{ widthPt: null, blocks: [{ type: 'paragraph' }] },
					],
				},
			],
		});
	...
	it('renders headers, comments, and notes instead of only warning about them', () => {
		...
		const model = parser.parse(
			makeDocxPackage({
				...
				'word/_rels/header1.xml.rels':
					'<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rHeaderImage" Target="media/header.png" /></Relationships>',
				'word/footer1.xml':
				...
				'word/comments.xml':
					'<w:comments xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:comment w:id="0" w:author="Alex"><w:p><w:r><w:t>Comment text</w:t></w:r></w:p></w:comment></w:comments>',
				'word/footnotes.xml':
		...
		expect(model.warnings).toEqual([
			'Document styles are missing; common Word style names will be inferred where possible.',
		]);
	});
	...
	it('parses styles, outline, numbering, links, metadata, and note references', () => {
		...
		const model = parser.parse(
			makeDocxPackage({
				...
				'word/footnotes.xml':
					'<w:footnotes xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:footnote w:id="2"><w:p><w:r><w:t>Note</w:t></w:r></w:p></w:footnote></w:footnotes>',
				'word/comments.xml':
					'<w:comments xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:comment w:id="4" w:author="Reviewer"><w:p><w:r><w:t>Looks good</w:t></w:r></w:p></w:comment></w:comments>',
				'word/document.xml': `<?xml version="1.0" encoding="UTF-8"?>
		...
		expect(model.blocks[2]).toMatchObject({
			type: 'paragraph',
			inlines: [{ type: 'text', hyperlink: 'https://example.com' }],
		});
	...
	it('parses page breaks, unsupported features, and embedded local images while blocking external images', () => {
		...
		const model = parser.parse(
			makeDocxPackage({
				'word/_rels/document.xml.rels':
					'<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rImage" Target="media/image1.png" /><Relationship Id="rExternal" Target="https://example.com/x.png" TargetMode="External" /></Relationships>',
		...
		expect(model.blocks).toContainEqual(
			expect.objectContaining({ type: 'unsupported', label: 'Embedded external document' }),
		);
	...
	it('formats supported list styles consistently', () => {
		const levels = ['bullet', 'lowerLetter', 'upperLetter', 'lowerRoman', 'upperRoman']
			.map(
				(format, index) =>
					`<w:abstractNum w:abstractNumId="${index}"><w:lvl w:ilvl="0"><w:numFmt w:val="${format}" /><w:lvlText w:val="${format === 'bullet' ? '&#8226;' : '%1.'}" /></w:lvl></w:abstractNum><w:num w:numId="${index + 1}"><w:abstractNumId w:val="${index}" /></w:num>`,
			)
			.join('');
		const body = [1, 2, 3, 4, 5]
			...
			.join('');
		const model = new DocxParser().parse(
			makeDocxPackage({
		...
		expect(model.blocks.filter((block) => block.type === 'paragraph').map((block) => block.list?.label)).toEqual([
			'•',
			'a.',
			'A.',
			'i.',
