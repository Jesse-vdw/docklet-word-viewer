...
describe('DocxPackageReader', () => {
	...
	it('enforces entry, expansion, and single-media safety limits before parsing', () => {
		expect(() => readWith(Array.from({ length: 1501 }, (_value, index) => info(`custom/${index}.xml`, 1)))).toThrow(/too many ZIP entries/);
		expect(() => readWith([info(CONTENT_TYPES_PATH, 100 * 1024 * 1024), info(DOCUMENT_XML_PATH, 60 * 1024 * 1024)])).toThrow(/safety limit/);
		expect(readWith([info(CONTENT_TYPES_PATH, 1), info(DOCUMENT_XML_PATH, 1), info('word/media/huge.png', 26 * 1024 * 1024)]).warnings[0]).toContain('Skipped oversized embedded media');
	});
});

function readWith(files: UnzipFileInfo[]) {
	return new DocxPackageReader((_data, options) => {
		for (const file of files) { options.filter(file); }
		return {
			[CONTENT_TYPES_PATH]: bytes('<Types />'),
