...
describe('WordViewerBridge', () => {
	...
	it('mounts a sandboxed iframe, resolves ready, posts commands, and dispatches callbacks', async () => {
		...
		expect(post).toHaveBeenCalledWith(expect.objectContaining({ bridgeId: 'test-bridge', type: 'searchChanged', query: 'word' }), '*');

		dispatchFrom(iframe, { channel: BRIDGE_CHANNEL, bridgeId: 'test-bridge', type: 'renderError', message: 'bad render' });
		dispatchFrom(iframe, { channel: BRIDGE_CHANNEL, bridgeId: 'test-bridge', type: 'searchResult', total: 4, active: 2 });
	...
	it('ignores messages that do not match the mounted bridge id', async () => {
		...
		dispatchFrom(iframe, { channel: BRIDGE_CHANNEL, bridgeId: 'wrong-bridge', type: 'searchResult', total: 2, active: 0 });
	...
	it('rejects mount when the iframe never reports ready and ignores invalid messages', async () => {
		vi.useFakeTimers();
		const onError = vi.fn();
		const bridge = new WordViewerBridge(document.body.appendChild(document.createElement('div')), { onError, onSearchResult: vi.fn() }, 'test-bridge');
		const ready = bridge.mount();
		window.dispatchEvent(new MessageEvent('message', { data: { channel: BRIDGE_CHANNEL, bridgeId: 'test-bridge', type: 'renderError', message: 'wrong source' } }));
		expect(onError).not.toHaveBeenCalled();
	...
	it('rejects a pending mount when the iframe is destroyed before ready', async () => {
		const bridge = new WordViewerBridge(document.body.appendChild(document.createElement('div')), { onError: vi.fn(), onSearchResult: vi.fn() });
		const ready = bridge.mount();
